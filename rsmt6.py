# ===========================================================================
# Radix Sparse Merkle Tree v6 (RSMT6) - region-committing nodes,
#                                       coherent consistency proofs
# ===========================================================================
#
# This is first, unoptimized attempt to implement 'coherent' updates where
# there is commitment to both depth and region which are opened and verified
# locally while checking if node split is valid.
#
# See rsmt6a.py for full story.
#
# Hashes
# ------
...
#   'N',  depth, region            : junction over the two preceding
#                                     stack entries (left, right).
#
# The verifier (verify_consistency below) runs
# a stack machine over pairs (old_digest, new_digest) extended with an
# advice tuple (depth, region) and a new/old flag, and checks EDGE
# COHERENCE at every junction-child edge where either endpoint is new:
#       advice present  AND  child_depth > d  AND  p||side <= child_region.
# Edges between two pre-existing nodes are exempt: their coherence was
# checked in the round that created them and is frozen by the hashes.
...
def advice_extends(delta, rho, d, p, side):
    """Check  p || side  <=  rho  (rho has delta bits), i.e. edge coherence."""
    if delta is None or delta <= d:
        return False
    return (rho >> (delta - d)) == p and ((rho >> (delta - d - 1)) & 1) == side
...
class SparseMerkleTree:
    ...
    def batch_insert(self, batch):
        ...
        Returns (applied_items, proof_stream)."""
        new_items = {}
    ...
    def _insert(self, node, items, lo, hi, parent_new, out):
        ...
        out.extend(["N", node.depth, node.region])
...
def verify_consistency(proof, old_root, new_root, batch, _=None):
    """Stack machine over (old_digest, new_digest, advice, flag),
    with edge-coherence checks at every junction-child edge where either
    endpoint is new."""
    if not batch:
    ...
    try:
        while pi < len(proof):
            ...
            if tag == "S":
                ...
                if h is None:
                    return False
                stack.append((h, h, None, None, "old"))

            elif tag == "O":
                d, p, lh, rh = proof[pi:pi + 4]; pi += 4
                h = hash_node(d, p, lh, rh)
                stack.append((h, h, d, p, "old"))

            elif tag == "OL":
                k, v = proof[pi:pi + 2]; pi += 2
                h = hash_leaf(k, v)
                stack.append((h, h, KEY_BITS, k, "old"))

            elif tag == "L":
                k, v = sorted_batch[bi]; bi += 1
                stack.append((None, hash_leaf(k, v), KEY_BITS, k, "new"))

            elif tag == "N":
                d, p = proof[pi:pi + 2]; pi += 2
                if not (0 <= d < KEY_BITS and 0 <= p < (1 << d if d else 1)):
                    return False
                rh0, rh1, rdelta, rrho, rflag = stack.pop()
                lh0, lh1, ldelta, lrho, lflag = stack.pop()
                ...
                # edge coherence (the new constraint of v6)
                for delta, rho, flag, side in (
                    (ldelta, lrho, lflag, 0),
                    (rdelta, rrho, rflag, 1),
                ):
                    if is_new or flag == "new":
                        if not advice_extends(delta, rho, d, p, side):
                            return False

                h1 = hash_node(d, p, lh1, rh1)
                stack.append((h0, h1, d, p, "new" if is_new else "old"))
...
if __name__ == "__main__":
    ...
    print("honest history: %d keys, all proofs verify" % len(recorded))
    ...
    attack_a = ["S", root, "L", "N", d_star, prefix(k, d_star)]
