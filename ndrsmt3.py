...
class SparseMerkleTree:
    ...
    def _build_pure_subtree(self, batch, start_bit):
        ...
        return NodeBranch(cp, self._build_pure_subtree(lb, split), self._build_pure_subtree(rb, split), split)
...
def _build_pure_hashes(batch, start_bit):
    ...
    return hash_node(_build_pure_hashes(lb, split), _build_pure_hashes(rb, split), split)
...
def synchronized_proof_eval(proof_iterator, batch, start_bit=0):
    ...
    if tag == 'B':
        if not batch:
            raise ValueError("Proof requested pure build 'B', but no items routed here.")
