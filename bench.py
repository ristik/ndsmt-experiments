...
def run_benchmark(depth=256, batch_size=10000, num_rounds=60):
    ...
    print(f"{'round':>5} {'batch':>5} {'total':>7} {'insert_s':>9} "
          f"{'verify_s':>9} {'ins/s':>7} {'mem_MB':>8} {'prf_MB':>10}", file=sys.stderr)
    ...
    for rnd in range(num_rounds):
        ...
        print(f"{rnd:5d} {len(batch):5d} {total_leaves:7d} {t_insert:9.3f} "
              f"{t_verify:9.3f} {ips:7.0f} {mem_mb:8.1f} {prf_size:8.1f}", file=sys.stderr)

    print(f"\nFinal: {total_leaves} leaves", file=sys.stderr)

if __name__ == "__main__":
    batch_size = int(sys.argv[1]) if len(sys.argv) > 1 else 10000
    num_rounds = int(sys.argv[2]) if len(sys.argv) > 2 else 600
